...
const coverArtImg     = document.getElementById('music-player-cover-art-img');
const heroLayerA      = document.getElementById('hero-layer-a');
const heroLayerB      = document.getElementById('hero-layer-b');
const headerEl        = document.getElementById('header');

// Tracks which hero layer is currently active so we always write to the inactive one
let activeHeroLayer   = 'none'; // 'a' | 'b' | 'none'
...
function loadTrack(index) {
  ...
  // Determine which layer is incoming (always the one not currently active)
  if (track.hero && heroLayerA && heroLayerB && headerEl) {
    const incoming = (activeHeroLayer === 'a') ? 'b' : 'a';
    const incomingEl = (incoming === 'a') ? heroLayerA : heroLayerB;

    // Set the new image on the hidden layer BEFORE making it visible
    // background-size:cover + background-position:center center (set in CSS)
    // guarantees fitted, centered, no-stretch at all sizes
    incomingEl.style.backgroundImage = `url('${track.hero}')`;

    // Swap active class — CSS transition handles the 0.75s crossfade
    headerEl.classList.remove('hero-a-active', 'hero-b-active');
    headerEl.classList.add(`hero-${incoming}-active`);
    activeHeroLayer = incoming;
  }

  // ── Cover art thumbnail swap ──────────────────────────────────
  if (coverArtImg && track.cover) {
    coverArtImg.classList.remove('cover-spin');
    void coverArtImg.offsetWidth;   // force reflow so animation re-triggers
    coverArtImg.classList.add('cover-spin');
    coverArtImg.src = track.cover;
    coverArtImg.alt = `${track.artist} — ${track.title} cover art`;
  }
...
function applyParallax() {
  ...
  blobNodes.forEach(blob => {
    ...
    // Clamp translateY to the parent's height so the blob never
    // extends the document beyond the section boundary.
    const parent = blob.el.parentElement;
    const maxTranslate = parent ? parent.offsetHeight * 0.3 : 200;
    const rawTranslate = scrollY * blob.rate;
